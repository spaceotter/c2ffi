 ...
 */
static void amendFromStream(clang::CompilerInstance &ci, std::stringstream &ss,
                            const std::string &name, const c2ffi::config &sys, clang::Sema &S) {
  if (sys.verbose) {
    ss.clear();
    ss.seekg(0);
    std::string line;
...
int main(int argc, char *argv[]) {
  ...
  if (sys.preprocess_only) {
    ...
    clang::DoPrintPreprocessedInput(ci.getPreprocessor(), os, ci.getPreprocessorOutputOpts());
    delete os;
    ...
    if (sys.to_namespace != "") sys.od->write_namespace(sys.to_namespace);

    std::unique_ptr<clang::Sema> S(new clang::Sema(ci.getPreprocessor(), ci.getASTContext(), *astc,
  ...
  if (sys.fail_on_error && main_error) return 1;
  return 0;
}
