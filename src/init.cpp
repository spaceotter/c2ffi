...
void c2ffi::add_include(clang::CompilerInstance &ci, const char *path, bool is_angled,
  ...
  if (auto dirent = fm.getDirectoryRef(path); dirent) {
    ...
    ci.getPreprocessor().getHeaderSearchInfo().AddSearchPath(lookup, is_angled);
  }
}

void c2ffi::add_includes(clang::CompilerInstance &ci, c2ffi::IncludeVector &includeVector,
                         bool is_angled, bool show_error) {
  for (auto &&include : includeVector) add_include(ci, include.c_str(), is_angled, show_error);
}
...
void c2ffi::init_ci(config &c, clang::CompilerInstance &ci) {
  ...
  TextDiagnosticPrinter *tpd = new TextDiagnosticPrinter(llvm::errs(), &*DiagOpts, false);
  IntrusiveRefCntPtr<clang::DiagnosticIDs> DiagID(new clang::DiagnosticIDs());
  clang::DiagnosticsEngine Diags(DiagID, &*DiagOpts, tpd);

  clang::driver::Driver Driver(
      c.c2ffi_binpath, c.arch.empty() ? llvm::sys::getDefaultTargetTriple() : c.arch, Diags);
  ...
  TargetInfo *pti =
      TargetInfo::CreateTargetInfo(ci.getDiagnostics(), ci.getInvocation().TargetOpts);
  ci.setTarget(pti);
  ...
  switch (pti->getTriple().getEnvironment()) {
    ...
    default:
      std::cerr << "c2ffi warning: Unhandled environment: '"
                << pti->getTriple().getEnvironmentName().str() << "' for triple '" << c.arch << "'"
                << std::endl;
  }

  if (c.declspec) lo.DeclSpecKeyword = 1;

  if (c.wchar_size != 0) lo.WCharSize = c.wchar_size;
  ...
  if (!c.nostdinc && hso.ResourceDir.empty()) hso.ResourceDir = CLANG_RESOURCE_DIRECTORY;
