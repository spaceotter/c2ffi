...
#define LANGSTANDARD(ident, name, lang, desc, features) \
  if (std == name) return clang::LangStandard::lang_##ident;
...
void c2ffi::process_args(config &config, int argc, char *argv[]) {
  ...
  for (;;) {
    ...
    switch (o) {
      ...
      case 'o': {
        if (output_specified) {
          std::cerr << "Error: You may only specify one output file" << std::endl;
          exit(1);
        }
      ...
      case 'S':
        ...
        if (config.std == clang::LangStandard::lang_unspecified) {
          std::cerr << "Error: unknown standard specified, --std=" << optarg << std::endl;
          exit(1);
        }
        break;
  ...
  if (stat(config.filename.c_str(), &buf) < 0) {
    std::cerr << "Error: No such file: " << config.filename << std::endl;
    exit(1);
...
void usage(void) {
  ...
  for (int i = 0;; i++) {
    ...
    if (OutputDrivers[i + 1].name) cout << ", ";
  }
...
c2ffi::OutputDriver *select_driver(std::string name, std::string outf, std::string inf) {
  ...
  for (int i = 0;; i++) {
    ...
    if (name == OutputDrivers[i].name) return OutputDrivers[i].fn(os);
  }
