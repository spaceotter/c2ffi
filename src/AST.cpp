...
void C2FFIASTConsumer::HandleNS(const clang::NamespaceDecl* ns) { HandleDeclContext(ns, ns); }
...
bool C2FFIASTConsumer::is_cur_decl(const clang::Decl* d) const { return _cur_decls.count(d); }
...
Decl* C2FFIASTConsumer::make_decl(const clang::EnumDecl* d, bool is_toplevel) {
  ...
  for (clang::EnumDecl::enumerator_iterator i = d->enumerator_begin(); i != d->enumerator_end();
       ++i) {
...
Decl* C2FFIASTConsumer::make_decl(const clang::ObjCInterfaceDecl* d, bool is_toplevel) {
  ...
  _cur_decls.insert(d);
  ObjCInterfaceDecl* r =
      new ObjCInterfaceDecl(d->getDeclName().getAsString(),
                            super ? super->getDeclName().getAsString() : "", !d->hasDefinition());

  for (clang::ObjCInterfaceDecl::protocol_iterator i = d->protocol_begin(); i != d->protocol_end();
       i++)
