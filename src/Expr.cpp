...
static best_guess macro_type(clang::CompilerInstance& ci, clang::Preprocessor& pp,
                             const char* macro_name, const clang::MacroInfo* mi,
                             StringSet* seen = NULL);

static best_guess num_type(clang::CompilerInstance& ci, const clang::Token& t) {
  llvm::StringRef sr(t.getLiteralData(), t.getLength());
  clang::NumericLiteralParser parser(sr, t.getLocation(), ci.getSourceManager(), ci.getLangOpts(),
                                     ci.getTarget(), ci.getDiagnostics());
...
static best_guess tok_type(clang::CompilerInstance& ci, clang::Preprocessor& pp,
                           const char* macro_name, const clang::Token& t, StringSet* seen) {
...
static best_guess macro_type(clang::CompilerInstance& ci, clang::Preprocessor& pp,
                             const char* macro_name, const clang::MacroInfo* mi, StringSet* seen) {
...
static void output_redef(clang::Preprocessor& pp, const char* name, const clang::MacroInfo* mi,
